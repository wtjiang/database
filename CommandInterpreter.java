...
class CommandInterpreter {
    ...
    void loadStatement() {
        ...
        String tableName = name();
        Table loadedTable = Table.readTable(tableName);

        _database.put(tableName, loadedTable);
        _input.next(";");
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            newTable = tableName();
            cond = conditionClause(table, newTable);
            return table.select(newTable, colNames, cond);
        } else {
            cond = conditionClause(table);
            return table.select(colNames, cond);
        }
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        if (_input.peek().equals("where")) {
            _input.next("where");
            condition.add(condition(tables));
    ...
    Condition condition(Table... tables) {
        Column tempCol = new Column(columnName(), tables);
        String relation = _input.next();
        Condition cond;
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String tempV = literal();
            cond = new Condition(tempCol, relation, tempV);
        } else {
            Column tempV = new Column(columnName(), tables);
            cond = new Condition(tempCol, relation, tempV);
        }
        return cond;
    }
